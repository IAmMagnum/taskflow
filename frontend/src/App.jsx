  ...
  const handleSaveTask = async (taskData) => {
    try {
      if (taskData.id) {
        ...
        await api.createTask({ ...taskData, board_id: activeBoardId });
      }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        {/* Title + Board Selector Dropdown + New Board Button */}
        <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center space-x-2">
            <select
              ...
              className="bg-gray-50 border border-gray-300 text-gray-800 text-xs font-semibold rounded-md px-2.5 py-1.5 outline-none cursor-pointer focus:ring-2 focus:ring-blue-500"
            >
              {boards.map((b) => (
            ...
            <button
              ...
              className="text-xs bg-gray-800 hover:bg-gray-900 text-white font-medium px-3 py-1.5 rounded-md transition shadow-sm"
            >
              + Board
            </button>
          </div>
        ...
        {/* Search, Filter, & Add Task controls */}
        ...
        <div className="flex gap-6 min-w-[768px] items-start">
          {board?.columns.map((column) => {
            ...
            return (
              <div
                key={column.id}
                className="flex-1 bg-gray-200/60 rounded-xl p-4 min-w-[280px]"
              >
                {/* Column Header + Count */}
